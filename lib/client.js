	...
	factory: (require) => {
		...
		function ComposerStatsBar(ctx, props) {
			...
			react.useEffect(() => {
				...
				const load = () => {
					Promise.all([
						fetch("/api/usage-stats/usage").then((r) => (r.ok ? r.json() : null)),
						fetch("/api/usage-stats/balance").then((r) => (r.ok ? r.json() : null)),
					]).then(([u, b]) => {
			...
			// Line 2 items: today, cumulative, cache hit, balance.
			const items = [];
			const key = todayKey();
			...
			const segs = [];
			items.forEach((it, i) => {
				if (i > 0) segs.push(react_jsx_runtime.jsx("span", { className: "usgx-n", children: "·" }));
				segs.push(react_jsx_runtime.jsx("span", { className: it.cls, children: it.text }));
			});
			if (items.length > 0) {
